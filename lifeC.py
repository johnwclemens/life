...
#        a = [[[0, pyglet.shapes.Rectangle(fri(i*w+x), fri(wh-h-j*h+y), fri(w), fri(h), color=self.DEAD[(i+j)%self.ncolors], batch=self.batch)] for i in range(c)] for j in range(r)]
#        print('addGrid() using nested list comprehension a={}=a\n'.format(a), file=DBG_FILE)
#        b1, b2 = zip(*[zip(*[[0, pyglet.shapes.Rectangle(fri(i*w+x), fri(wh-h-j*h+y), fri(w), fri(h), color=self.DEAD[(i+j)%self.ncolors], batch=self.batch)] for i in range(c)]) for j in range(r)])
#        print('addGrid() using nested list comprehension b1={}=b1\nb2={}=b2\n'.format(b1, b2), file=DBG_FILE)
...
#        self.data, self.cells = list(zip(*[list(zip(*[(0, pyglet.shapes.Rectangle(fri(i*w+x), fri(wh-h-j*h+y), fri(w), fri(h), color=self.DEAD[(i+j)%self.ncolors], batch=self.batch)) for i in range(c)])) for j in range(r)]))
#        print('addGrid() tmp[0]={} tmp[1]={}'.format(tmp[0], tmp[1]), file=DBG_FILE)
#        self.data, self.cells = zip(*tmp)
#        print('addGrid() using nested list comprehension data={}\ncells={}\n'.format(self.data, self.cells), file=DBG_FILE)
